...
export async function perStation(args, httpsAgent){
    let station_id = args.station;
    ...
    fs.readFile(file, 'utf8', (err, TOKEN) => {
        if (err) {
            ...
            if (station_id && datefrom && dateto) {
                let url;
                if(format != "csv") {
                    url = `https://localhost:8765/evcharge/api/SessionPerStation/${station_id}/${datefrom}/${dateto}`;
                }
                else{
                    url = `https://localhost:8765/evcharge/api/SessionPerStation/${station_id}/${datefrom}/${dateto}?format={csv}`;
                }
                const options = {
                    httpsAgent,
                    method: 'GET',
