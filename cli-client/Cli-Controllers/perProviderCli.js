...
export async function perProvider(args, httpsAgent){
    ...
    fs.readFile(file, 'utf8', (err, TOKEN) => {
        if (err) {
            ...
            if (provider_id && datefrom && dateto) {
                let url;
                if(format !== "csv") {
                    url = `https://localhost:8765/evcharge/api/SessionPerProvider/${provider_id}/${datefrom}/${dateto}`;
                }
                else{
                    url = `https://localhost:8765/evcharge/api/SessionPerProvider/${provider_id}/${datefrom}/${dateto}?format={csv}`;
                }
                const options = {
                    httpsAgent,
                    method: 'GET',
